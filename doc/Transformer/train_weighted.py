...
def train_w_weight(model, epochs, trainloader, valloader, testloader, optimizer, criterion, save_path, early_stopping = None, save = True):
    """
    Train a neural network and save the best model on accoding to its performance on validation dataloader
...
def train_transformer_iter(iterations = 3, method = "attn", size = "huge", early_stopping = None, save = True):
    """
    Train a transformer model 5 times with different train test split. 
